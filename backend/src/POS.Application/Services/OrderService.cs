...
/// <summary>
/// Core checkout flow: builds the order, decrements stock, and charges each
/// tender through the matching IPaymentProcessor (payment orchestration layer).
/// Supports split tenders (e.g. part cash, part card) per the plan.
/// </summary>
public class OrderService
{
    ...
    public async Task<OrderResponse> CreateOrderAsync(Guid tenantId, Guid cashierUserId, CreateOrderRequest request, CancellationToken ct = default)
    {
        ...
        foreach (var tender in request.Tenders)
        {
            ...
            var result = await processor.ChargeAsync(tenantId, tender.Amount, "USD", paymentToken: null, ct);
