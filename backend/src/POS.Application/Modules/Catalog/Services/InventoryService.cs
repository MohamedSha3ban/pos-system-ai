...
public class InventoryService
{
    ...
    public async Task<List<InventoryItemDto>> GetAllAsync(Guid tenantId, Guid? locationId = null, CancellationToken ct = default)
    {
        ...
        var locations = await _db.Locations.Where(l => l.TenantId == tenantId).ToListAsync(ct);
    ...
    {
        var item = await _db.InventoryItems.FirstOrDefaultAsync(i => i.TenantId == tenantId && i.Id == inventoryItemId, ct);
